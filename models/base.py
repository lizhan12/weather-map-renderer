...
class ItemImg(BaseModel):
    ...
    publisher_location: str | None = Field('0.7,0.0,0.3,0.15', description="发布单位位置 eg:0,0,0.28,0.25 左/下/宽/高,百分比")
    mesh_padding: str | None = Field('0.0, 0.0, 0.0, 0.0', description="距离边框的内边界，步长单位经纬度,eg:0.1,0.1,0.1,0.1 上/右/下/左")
    ...
    interpolation_method: str = Field("rbf", description="插值方式: rbf(默认), idw(反距离加权), cressman(Cressman客观分析)")
    bounds_lines: str | None = Field(None,
                                           description="边界线,传入str, el:'2,1.5,0.7'表示边界线宽为2,区县为1.5,镇为0.7，必须3个数")
    bounds_colors: str | None = Field(None,
                                           description="边界线,传入str, el:'#333,#333, #666,第一个'表示边界线,第二区县为,第三个镇，必须3个数")
